...
# def convert_svg_to_base64(svg_string):
#     # Convert SVG string to ReportLab Graphics object
#     drawing = svg2rlg(io.BytesIO(svg_string.encode('utf-8')))
    
#     # Create a BytesIO buffer to store the PNG data
#     buffer = io.BytesIO()
    
#     # Render the drawing as a PNG into the buffer
...
class Evaluator:
    ...
    def evaluate(self, agent, dataset, config, specific_ids):
        print("[EVALUATOR] evaluate started =====")
        use_logs = False
        evaluation_details = []
        ...
        if "logs" in config:
            ...
            try:
                ...
                print("Name logging error s coming from here")
                print(e)

        for i, instance in enumerate(dataset.benchmark):
            if instance['id'] in specific_ids:
                print("Checking ID: ", instance['id'])
                # if i==10:
                #     print("BREAKING here reached the limit =====")
                #     break
                print("*"*50 +"INSTANCE STARTED"+ "*"*50)
                codes = []
                instance_results = []
                nl_queries = instance['nl_queries']
                tables = instance['tables']
                ...
                if use_logs:
                    instanceFolder = os.path.join(config['logs'], instance['id'])
                    path = os.path.join(instanceFolder, "result.json")
                    # print("result.json path")
                    # print(path)
                    if os.path.exists(path):
                        print("Enterring path exists if block")
                        with open(path, "r") as f:
                            data = json.load(f)
                            # print("ALREADY PRESENT RESULTS JSON Data:")
                            # print(data)
                            if ("codes" in data) and ('evaluations' in data):
                                instance_results = []
                                for query_result in data['evaluations']:
                                ...
                                # print("BELOW INSTANCE FOLDER ALREADY PRESENT:")
                                # print(instanceFolder)
                                # print("Not Generating the new Visual Code")
                                continue
                    else:
                        logging.info(f"Instance ({instance['id']}) evaluation began.")
                        isExists = os.path.exists(instanceFolder)
                ...
                for index in range(len(nl_queries)):
                    ...
                    if index< len(codes):
                        ...
                        try:
                            ...
                            print(f"Error generating code for NL query [{index+1}]: {e}")
                            code = None
                            context = {}
                    ...
                    if code is None:
                        results = [CheckResult(answer=False,
                                                aspect="code generation",
                                                  rationale="Code generation failed.")]
                    else:
                        if use_logs:
                                results = self.validity_check(
                                    code, context, agent, os.path.join(instanceFolder, f"{index}.svg")
                                )
                        else:
                            results = self.validity_check(code, context, agent)
                ...
                evaluation_details.append(EvaluationDetail(instance['id'], instance_results))

                if use_logs:
                    logging.info(f"Instance ({instance['id']}) evaluation finished.")
                    print(f"Completed evaluation for instance: {instance['id']}")

                    instance_results = [[result.get_json() for result in results] for results in instance_results]

                    instance_id = instance["id"]  # Assuming `instance` is defined in your code
                    log_path = os.path.join(log_folder, instance_id)
                ...
                print("*"*50 + "INSTANCE RESULTS" + "*"*50)
                print(instance_results)
                print("\n")
                print("*"*50 +"INSTANCE COMPLETED"+ "*"*50)
                print("\n")
        print("EVALUATION DETIALS:")
        print(evaluation_details)
        print("[EVALUATOR] evaluate completed =====")
        return EvaluationResult(dataset, evaluation_details)
